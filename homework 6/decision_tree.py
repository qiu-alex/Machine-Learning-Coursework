...
class DecisionTree:
    ...
    def learn_tree(self, examples):
        ...
        if num_samples >= self.min_leaf_count:
            ...
            #check subtree size
            leaf_count = True if len(best_split['child_lt']) > self.min_leaf_count and len(best_split['child_ge']) > self.min_leaf_count else False
            #create decision node based on best split
            if best_split['info_gain'] > 0 and leaf_count:
        ...
        #find pred class    
        leaf_samples = [sample[self.class_name] for sample in examples]
    ...
    def find_best_split(self, examples):
        ...
        test_attr = list(examples[0])
        test_attr.remove(self.class_name)
        test_attr.remove(self.id_name)
        #keys = [k for k in test_attr]
        for attr in test_attr:
            ...
            for threshold in values:
                ...
                if len(child_lt) > 0 and len(child_ge) > 0:
                    #calculate info gain
    ...
    def split(self, examples, attr, threshold):
        ...
        for sample in examples:
            #if sample missing field add to child_lt
            if sample[attr] == None:
    ...
    def entropy(self, examples):
        #find probabilities of class labels
        values = [sample[self.class_name] for sample in examples]
        #get all unique labels
        labels = list(set(values))
        entropy = 0
        for l in labels:
            instances = [v for v in values if v == l]
    ...
    def predict(self, example, node):
        ...
        if not (hasattr(node, 'test_attr_name')):
            ...
            #if sample does not have given attribute, move to less than subtree
            if example[test_attr] == None:
                return self.predict(example, node.child_lt)
            elif example[test_attr] >= node.test_attr_threshold:
